...
public class ThrowRecorder
{
    ...
    private final static int MINIMUM_DATA_THRESHOLD = 8;//Minimum data needed before predictions start being made
...
* throw recorded. Then the player's after. Pairs are recorded as CH (Where C
* is the computer's throw and H is the humans throw)
* @param computerThrow Throw generated by the computer
* @param userThrow Throw selected by the player
*/
    public void recordThrows(char computerThrow, char userThrow)
    {
     //Updates the total amount of throws that have been recorded
        ...
        for(int i=1;i<MAX_SEQUENCE_LENGTH-1;i++)
        {
            try
            {
             //Updates the hash table with the given sequence
                updateSequenceTable(currentSequence.substring(i, MAX_SEQUENCE_LENGTH));
            }
            catch(StringIndexOutOfBoundsException e)
            {
                //Handles special cases where string length is not long enough
                //for standard operations to be applied
